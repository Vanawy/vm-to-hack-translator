...
`Project 7` of www.nand2tetris.org
...
1. Use the XxxVME.tst script to execute the test program Xxx.vm on the VM emulator. This will
   ...
   again, interactively and step-wise, one VM command at a time. Inspect the impact of each VM command 
    on the host RAM locations that store the stack and the segments (bottom right of the VM simulator’s 
    GUI). The assembly code that your VM translator generates should have the same impact on the Hack RAM.
2. Use your partially implemented translator to translate Xxx.vm. The result should be a text file
   named Xxx.asm, containing the Hack assembly code generated by your translator.
3. Inspect the generated Xxx.asm code produced by your translator. If there are visible syntax (or
   other) errors, debug and fix your translator.
4. Use the supplied Xxx.tst script to load, run and test, on the CPU emulator, the Xxx.asm program
   ...
   properly.
   References
   The VM emulator is used in this project for the first time. It’s important to get acquainted with this
   tool, for this project and for all the subsequent projects in the course. We also provide references
   to the CPU emulator, for completeness. These references are less important, since by now you
   probably feel familiar with this tool.